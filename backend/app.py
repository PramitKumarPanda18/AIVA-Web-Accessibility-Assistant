...
class ConnectionManager:
    ...
    async def broadcast(self, message: str):
        ...
        async def send_to_client(websocket: WebSocket):
            try:
                # Use a small timeout to prevent slow clients from blocking
                await asyncio.wait_for(websocket.send_text(message_data), timeout=0.5)
                return None
            except Exception:
                return websocket

        # Run all sends in parallel
        failed_connections = await asyncio.gather(
...
# Create FastAPI app
app = FastAPI(
    title="AIVA - Web Accessibility Assistant API",
    description="AI-powered voice shopping assistant platform",
    version="2.0.0",
    lifespan=lifespan,
)
...
async def create_order(request: CreateOrderRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Validate retailer (soft check — log warning but don't block)
        ...
        if not retailer_urls:
            logger.warning(f"Retailer '{request.retailer}' has no configured URLs — order will proceed with defaults.")
...
async def start_voice_conversation():
    ...
    try:
        ...
        # Start the conversation (skip backend TTS generation for speed)
        result = await voice_service.start_conversation(conversation_id, generate_audio=False)
        
        # Determine audio format from service config
        audio_format = "mp3" if voice_service.voice_provider == "polly" else "mp3" # Default to mp3 for Polly/Native fallback
...
@app.post("/api/voice/conversation/{conversation_id}/process_text")
async def process_text_input(
    conversation_id: str,
    text_data: Dict[str, str]
):
    """Process text input (STT transcript) and return voice response"""
    try:
        from services.voice_service import get_voice_service
        ...
        # Process the conversation (generate_audio=False for maximum speed)
        result = await voice_service.process_conversation(conversation_id, user_text, generate_audio=False)
        
        # Add the original user text to the result for synchronization
        result["user_text"] = user_text
        
        # Frontend uses Native Browser TTS, so we don't need to generate Polly audio here
        # This saves ~2 seconds of processing time per turn!
        result["audio"] = None
...
async def submit_voice_order(
    ...
    try:
        ...
        # Map contact to phone/email if possible, and street to address parts
        contact_info = order_data.get("contact", "")
        if "@" in contact_info and not order_data.get("customer_email"):
            order_data["customer_email"] = contact_info
        elif not order_data.get("phone"):
            order_data["phone"] = contact_info

        # Auto-fill missing required fields with mock data for Voice Orders
        # since we don't want the AI to interrogate the user for 10 minutes
        order_data.setdefault("customer_email", "voice_user@example.com")
        order_data.setdefault("phone", "555-0199")
        order_data.setdefault("city", "Seattle")
        order_data.setdefault("state", "WA")
        order_data.setdefault("postal_code", "98109")

        # Now check if core fields are still completely missing
        required_fields = {
        ...
        if missing_fields:
            raise HTTPException(
                status_code=400,
                detail=f"Missing required fields for automation: {', '.join(missing_fields)}. Please complete voice conversation first."
            )
        ...
        order_request_data = {
            "retailer": order_data.get("retailer", "amazon"),
            "automation_method": automation_method,
            "ai_model": ai_model or order_data.get("ai_model", "anthropic.claude-sonnet-4-20250514-v1:0"),
            "product": {
...
# ============================================================================
# SCAN TO ORDER - Image Recognition via Amazon Nova Lite Vision
# (NOT part of voice agent backend)
# ============================================================================

class ScanImageRequest(BaseModel):
    image_base64: str = Field(..., description="Base64-encoded image data (with or without data URI prefix)")

@app.post("/api/scan-image")
async def scan_image(request: ScanImageRequest):
    """Identify a product from an uploaded image using Amazon Nova Lite vision"""
    try:
        import boto3
        import base64
        import re

        # Strip data URI prefix if present and get media type
        raw = request.image_base64
        media_type = "image/jpeg"
        prefix_match = re.match(r'data:(image/[a-zA-Z+]+);base64,', raw)
        if prefix_match:
            media_type = prefix_match.group(1)
            raw = raw.split(',', 1)[1]

        # Map media type to format string Nova expects
        ...
        # Use the Converse API which handles image bytes properly
                    ...
                    "content": [
                        ...
                        },
                        {
                            "text": (
                                ...
                                "Look at this image and identify the product shown. "
                                "Respond with ONLY the product name, brand (if visible), and size/quantity in a single line. "
                                "Example: 'Tide Original Liquid Laundry Detergent (92 oz)'. "
                                "If you cannot identify a product, respond with 'Unknown product — please try a clearer image'."
                            ),
                        },
                    ],
                }
        ...
        product_name = response["output"]["message"]["content"][0]["text"]

        return {"success": True, "product": product_name.strip()}

    except Exception as e:
        logger.error(f"Scan image failed: {e}", exc_info=True)
        return {"success": False, "product": f"Could not identify product: {str(e)[:100]}"}


# ============================================================================
# MULTILINGUAL TRANSLATION API (Keeps voice agent logic in English)
# ============================================================================

class TranslateRequest(BaseModel):
    text: str
    target_language: str = "en" # 'en' means translate TO English (from whatever user spoke)

@app.post("/api/translate")
async def translate_text(request: TranslateRequest):
    """Translate text quickly using Nova Lite"""
    try:
        import boto3
        aws_key = os.environ.get("AWS_ACCESS_KEY_ID")
        aws_secret = os.environ.get("AWS_SECRET_ACCESS_KEY")
        ...
        direction_prompt = (
            f"Translate the following text into English. Respond ONLY with the english text, nothing else."
            if request.target_language == "en" else
            f"Translate the following text into {request.target_language}. Respond ONLY with the translated text, nothing else."
        )
        ...
        response = bedrock.converse(
            ...
            messages=[
                {
                    "role": "user",
                    "content": [{"text": f"{direction_prompt}\n\nText: {request.text}"}],
                }
            ],
        ...
        return {"success": True, "translated_text": translated.strip()}

    except Exception as e:
        logger.error(f"Translation failed: {e}", exc_info=True)
        return {"success": False, "translated_text": request.text} # Fallback to original
