...
class OrderQueue:
    ...
    async def _process_order(self, order: Order):
        ...
        try:
            ...
            # INTENTIONAL RANDOM FAILURE FOR UI TESTING (~20% chance)
            if random.random() < 0.20:
                logger.warning(f"Simulating intentional failure for order {order.id}")
                # Simulate a bit of processing time
                self.db_manager.update_order_status(
                    order_id=order.id,
                    status=OrderStatus.IN_PROGRESS,
                    progress=15,
                    current_step="Authenticating with retailer...",
                )
                ...
                error_messages = [
                    ...
                    "Item went out of stock during checkout",
                    "Delivery address cannot be validated",
                    "Session expired abruptly"
                ]
                ...
                self.db_manager.update_order_status(
                    ...
                    error_message=f"Simulation: {chosen_error}",
                )
                return
            else:
                # MOCK SUCCESS FOR REMAINING 80%
                logger.info(f"Simulating intentional SUCCESS for order {order.id} to bypass AWS agent")
                self.db_manager.update_order_status(
                    order_id=order.id,
                    status=OrderStatus.IN_PROGRESS,
                    progress=20,
                    current_step="Navigating to product page...",
                )
                await asyncio.sleep(2)
                self.db_manager.update_order_status(
                    order_id=order.id,
                    status=OrderStatus.IN_PROGRESS,
                    progress=60,
                    current_step="Adding to cart and checking out...",
                )
                ...
                self.db_manager.update_order_status(
                    ...
                    status=OrderStatus.IN_PROGRESS,
                    progress=90,
                    current_step="Confirming payment...",
                )
                await asyncio.sleep(2)
                ...
                self.db_manager.update_order_status(
                    ...
                    order_confirmation_number=f"MOCK-{order.id[:8].upper()}",
                    tracking_number=f"TRK{random.randint(100000000, 999999999)}",
                    estimated_delivery=(datetime.now(timezone.utc)).strftime("%Y-%m-%d"),
                )
                return
    ...
    async def add_order(
        ...
        **kwargs,
        ...
        try:
            # Validate inputs (soft check)
            retailer_urls = self.settings_service.get_retailer_urls(retailer)
            if not retailer_urls:
                logger.warning(f"Retailer '{retailer}' has no configured URLs — order will proceed with defaults.")
