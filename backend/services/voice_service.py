...
try:
    ...
    try:
        ...
        BIDIRECTIONAL_STREAMING_AVAILABLE = False # Force disable due to instability
    except ImportError:
...
class VoiceService:
    ...
    def __init__(
        ...
        # ALWAYS prefer env variable, then parameter, then hardcoded default
        self.voice_model = voice_model or os.getenv("VOICE_MODEL", "us.amazon.nova-lite-v1:0")
        logger.info(f"VoiceService initialized with model: {self.voice_model}")
        ...
        # We will lazy-initialize AWS clients when they are actually needed
        # This prevents the 30-second EC2 metadata (IMDS) timeout bug when running 
        # locally without explicit STS tokens.
        ...
        try:
            ...
            logger.error("FFmpeg is NOT installed or not in PATH. Audio processing will likely fail.")
            logger.error("Please install FFmpeg: https://ffmpeg.org/download.html")

    async def start_conversation(self, conversation_id: str, generate_audio: bool = False) -> Dict[str, Any]:
        ...
        # Generate speech from text only if explicitly requested
        audio_response = None
        if generate_audio:
    ...
    async def pre_warm_model(self):
        """Pre-warm the Bedrock model to reduce first-hit latency"""
        try:
            ...
            logger.info(f"Pre-warming Bedrock model: {self.voice_model}")
            # Send a tiny silent ping in a separate thread
            model_id = self.voice_model if self.voice_model and self.voice_model.strip() else "us.amazon.nova-lite-v1:0"
            ...
            logger.info("Bedrock model pre-warmed successfully")
        except Exception as e:
            logger.warning(f"Failed to pre-warm Bedrock: {e}")
    ...
    async def _process_speech_converse_stream(
        ...
        try:
            # Ensure bedrock_runtime is initialized to avoid NoneType errors or IMDS timeouts
            if not self.bedrock_runtime:
                self.bedrock_runtime = boto3.client(
                    'bedrock-runtime', 
                    region_name=self.region,
...
Your responses must be natural spoken language, not JSON or structured format."""
    ...
    async def text_to_speech_polly(self, text: str) -> str:
        ...
        try:
            ...
            # Call Polly in a separate thread
    ...
    async def process_conversation(
        ...
        try:
            # Ensure bedrock_runtime is initialized (with explicit credentials to avoid IMDS timeout)
            if not self.bedrock_runtime:
                logger.info("bedrock_runtime was None, initializing now...")
                self.bedrock_runtime = boto3.client(
                    'bedrock-runtime',
                    region_name=self.region,
                    aws_access_key_id=os.getenv('AWS_ACCESS_KEY_ID'),
            ...
            # Use Amazon Nova Lite (works immediately, no use-case form needed)
            model_id = self.voice_model if self.voice_model and self.voice_model.strip() else "us.amazon.nova-lite-v1:0"
            logger.info(f"Calling Bedrock model: {model_id} with input: {user_input[:100]}")
            # Execute synchronous boto3 call via asyncio.to_thread to prevent event loop blocking
            response = await asyncio.to_thread(
                self.bedrock_runtime.converse,
                modelId=model_id,
                messages=[
            ...
            logger.info(f"Bedrock response received: {assistant_message[:200]}")
            ...
            if "conversation_history" in conversation:
                conversation["conversation_history"].append({"role": "user", "text": user_input})
                conversation["conversation_history"].append({"role": "assistant", "text": parsed_response["response_text"]})
            
            # Generate audio only if requested
            audio_data = None
            if generate_audio:
                audio_data = await self.text_to_speech(parsed_response["response_text"])

            return {
                "text": parsed_response["response_text"],
                "assistant_text": parsed_response["response_text"],
                "audio": audio_data,
                "state": parsed_response["next_state"],
                "order_data": parsed_response["order_data"],
                "ready_to_submit": parsed_response.get("ready_to_submit", False)
            }
            
        except Exception as e:
            import traceback
            logger.error(f"Conversation processing FAILED: {type(e).__name__}: {e}")
            logger.error(traceback.format_exc())
            return {
                "text": "I'm sorry, I had trouble understanding that. Could you please repeat?",
                "assistant_text": "I'm sorry, I had trouble understanding that. Could you please repeat?",
                "audio": None,
                "state": current_state,
...
Required order fields (you must ask for them step-by-step if missing):
- product_name: General name of the product (e.g., laptop, milk, shoes)
- brand: The specific brand of the product (e.g., Apple, Dell, Nike)
- specifications: Specific details like size, color, model, or features (e.g., 16GB RAM, Blue, Size 10)
- quantity: How many they want to order (e.g., 1, 2, 5)
...
2. **CRITICAL**: When the user first mentions a product, you MUST immediately ask them specifying questions about the brand, specifications (model/size/color), and quantity before moving on to their personal details.
3. Validate and confirm important details
4. When you have all required information, ask for final confirmation
5. **CRITICAL**: Once the user has confirmed the final order, you MUST set "ready_to_submit": true in your JSON output so the system can unlock the Submit button.
6. Respond in a conversational tone

Your response must be ONLY valid JSON matching this schema, with NO markdown formatting, no ```json wrappers, and NO conversational text before or after the JSON:
{
  "response_text": "Your natural language conversational reply to the user (what you want to say out loud)",
  "order_data": {"product_name": "...", "brand": "...", "specifications": "...", "quantity": "...", "customer_name": "...", "street": "...", "contact": "..."}, 
  "next_state": "state_name (greeting, collecting_product, collecting_customer, collecting_shipping, confirming, ready)",
...
"""
    ...
    def _parse_assistant_response(self, assistant_message: str, current_order_data: Dict) -> Dict[str, Any]:
        """Parse AI response and extract structured data using regex to handle markdown/filler"""
        import re
        try:
            # 1. Strip markdown wrappers if AI still generates them
            clean_msg = re.sub(r'```json\s*', '', assistant_message)
            clean_msg = re.sub(r'```\s*', '', clean_msg)
            
            # 2. Fix potential trailing comments the AI might add to the boolean
            clean_msg = re.sub(r'("ready_to_submit":\s*(?:true|false))[^,\}\n]*([,\}\n])', r'\1\2', clean_msg, flags=re.IGNORECASE)

            # 3. Try to find JSON block directly
            json_match = re.search(r'\{[\s\S]*\}', clean_msg)
            if json_match:
                json_str = json_match.group(0)
                parsed = json.loads(json_str)
                
                # Merge order data gracefully
                updated_order = parsed.get("order_data", {})
                if not isinstance(updated_order, dict):
                    updated_order = {}
                merged_order_data = {**current_order_data, **updated_order}
                
                return {
                    "response_text": parsed.get("response_text", "I'm sorry, I misunderstood."),
                    "order_data": merged_order_data,
                    "next_state": parsed.get("next_state", "collecting"),
                    "current_field": parsed.get("current_field"),
                    "ready_to_submit": bool(parsed.get("ready_to_submit", False))
                }
            ...
            logger.warning(f"Failed to parse JSON: {e} - Raw msg: {assistant_message}")
            return {
                "response_text": "I received your message, but had trouble processing my data. Let's continue.",
                "order_data": current_order_data,
                "next_state": "collecting",
                "ready_to_submit": False
            }
...
def get_voice_service(
    ...
    # ALWAYS read VOICE_MODEL from .env first — config_manager was overriding this
    env_model = os.getenv("VOICE_MODEL", "us.amazon.nova-lite-v1:0")
    ...
    if config_manager:
        ...
        # Use env variable as primary source for model (not database config)
